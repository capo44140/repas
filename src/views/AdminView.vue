  ...
  try {
    ...
    const repasData = {
      ...
      ingredients: form.value.ingredients
        ? form.value.ingredients.split("\n").filter((i) => i.trim())
        : [],
      instructions: form.value.instructions
        ? form.value.instructions.split("\n").filter((i) => i.trim())
        : [],
    };
...
const editRepas = (repas) => {
  form.value = {
    ...repas,
    ingredients: Array.isArray(repas.ingredients)
      ? repas.ingredients.join("\n")
      : "",
    instructions: Array.isArray(repas.instructions)
      ? repas.instructions.join("\n")
      : "",
  };
...
const handleImageError = (event) => {
  ...
  event.target.parentElement.innerHTML = `
            <div class="w-full h-full flex items-center justify-center rounded-md" :class="bgClasses.tertiary">
          <svg class="w-8 h-8" :class="iconClasses.muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  try {
    ...
    const response = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?apiKey=${SPOONACULAR_API_KEY}&query=${encodeURIComponent(
        repas.nom
      )}&number=1&addRecipeInformation=true`
    );
    ...
    if (data.results && data.results[0]) {
      ...
      const updatedRepas = {
        ...
        difficulte: recipeData.dishTypes?.includes("easy")
          ? "Facile"
          : recipeData.dishTypes?.includes("medium")
          ? "Moyen"
          : "Difficile",
        cout:
          recipeData.pricePerServing < 100
            ? "Bas"
            : recipeData.pricePerServing < 200
            ? "Moyen"
            : "Élevé",
      ...
      await loadRepas();
      showNotification(
        "Recette mise à jour avec succès depuis Spoonacular",
        "success"
      );
    } else {
...
const updateFromOpenFoodFacts = async (repas) => {
  try {
    ...
    if (data.products && data.products.length > 0) {
      ...
      await loadRepas();
      showNotification(
        "Informations nutritionnelles mises à jour avec succès depuis Open Food Facts",
        "success"
      );
    } else {
      showNotification(
        "Aucune donnée trouvée dans Open Food Facts pour cette recette",
        "error"
      );
    }
  } catch (error) {
    console.error(
      "Erreur lors de la mise à jour depuis Open Food Facts:",
      error
    );
    showNotification(
      "Erreur lors de la mise à jour des informations nutritionnelles",
      "error"
    );
  } finally {
...
<template>
  <div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
      <h1 class="text-2xl font-bold" :class="textClasses.primary">
    ...
    <div class="rounded-lg shadow-md p-6 mb-8" :class="cardClasses.background">
      <h2 class="text-xl font-semibold mb-4" :class="textClasses.primary">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label
              class="block text-sm font-medium text-gray-700 dark:text-gray-300"
              >Temps de préparation (min)</label
            >
            <input
                ...
                >
                  <div class="flex space-x-2">
                    ...
                    >
                      ...
                      <span v-if="isUpdatingOpenFood" class="animate-spin ml-1"
                        >⟳</span
                      >
                    </button>
                  </div>
